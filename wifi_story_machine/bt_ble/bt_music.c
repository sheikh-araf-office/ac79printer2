...
#define POWERON_AUTO_CONN_TIME    18  //�?机去回连的时�?,要取6的�?�数
...
struct app_bt_opr {
    ...
    u8 call_flag : 1;	// 1-由于蓝牙打电话命令切回蓝牙模�?
    u8 exit_flag : 1;	// 1-可以�?出蓝牙标�?
    u8 enable    : 1;
    u8 mute      : 1;
...
#define SBC_ZERO_TIME_MS			500		//静音多长时间认为已经�?�?
...
{
    //取消默认蓝牙定时发�?�电量给手机，需要更新电量给手机使用USER_CTRL_HFP_CMD_UPDATE_BATTARY命令
    /*电量协议的是0-9个等级，请比例换�?*/
...
static void bredr_handle_register(void)
{
    ...
    ///电量显示获取电量的接�?
    get_battery_value_register(bt_get_battery_value);   /*电量显示获取电量的接�?*/
#endif
    ///被测试盒链接上进入快速测试回�?
...
//    __set_a2dp_auto_play_flag(1); /* 连接高级音频后自动播放歌�? */
  //  __set_support_msbc_flag(1);
    ...
    __set_simple_pair_flag(1); //是否打开�?易配对功能，打开后不�?要输入pincode
    ////设置蓝牙加密的level
    ...
    ////设置蓝牙设备类型
    printf("sang:set class of device\n");
    //__change_hci_class_type(BD_CLASS_CAR_AUDIO);
    __change_hci_class_type(0x40680); // @20230706 Alke
...
#endif
	transport_spp_flow_cfg(); // @202402181431 Alke ����ɣ�罨������
}
...
//    transport_spp_flow_cfg(); // @20230706 Sang
    btstack_init();
}
...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
    ...
    case BT_STATUS_FIRST_CONNECTED:
        printf("sang:BT_STATUS_CONNECTED");
        ...
        if (get_total_connect_dev() == 1) {   //当前有一台连接上�?
...
#endif
            user_send_cmd_prepare(USER_CTRL_HFP_CALL_CURRENT, 0, NULL); //发命令获取电话号�?
        } else {
    ...
    case BT_STATUS_SNIFF_STATE_UPDATE:
        log_info(" BT_STATUS_SNIFF_STATE_UPDATE %d", bt->value);    //0�?出SNIFF
        if (bt->value == 0) {
