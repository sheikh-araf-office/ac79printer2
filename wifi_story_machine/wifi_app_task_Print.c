...
unsigned long GetEmptySize(void)
{
	...
	{
		return MAXBufferReceiveSize+MEP.SocketPread-MEP.SocketPwrite;
	}
	else
	{
...
/*		for(i=0; i++; i<sizebytes)
		...
		if((MEP.SocketPwrite+sizebytes)==MAXBufferReceiveSize)
			...
			MEP.SocketPwrite=MEP.SocketPwrite+sizebytes;
	}
	else
	{
//printf("s2");
...
//printf("0");
		...
		{
			// 只有有数据进来，才会固定当前数据通道
			if((DataFromChannel!=0)&&(DataFromChannel!=2))
				return;
			DataFromChannel = 2;
...
/*static int cc;
    ...
    while(1)
    {
		...
		else
		{
			...
			SppSend[16]='0'+cqi%10;
			//BT_RevC->DataOut(&SppSend, 17);
			//tcp_send_data_sang(&SppSend, 17);
			//printf(&SppSend, 17);
			// 不能发送数据，否则出错
		}*/
/*		if(ReturnMsg == 0xff)
