...
#if USE_CUT_TO_LCD
    YUV420p_Cut(buf, src_w, src_h, yuv->data, yuv->len, (src_w - out_w) / 2, out_w, (src_h - out_h) / 2, out_h);//裁剪取屏大小数据
#else
...
extern int jpeg2yuv_open(u8 usb_id);
...
void video0_display_start(void)
{
    thread_fork(YUV2RGB_TASK_NAME, 6, 512, 128, &__this->pid, yuv2rgb_task, NULL);
