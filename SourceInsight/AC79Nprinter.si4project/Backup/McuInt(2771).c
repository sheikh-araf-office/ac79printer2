...
//printf("BI1");
			...
			case sPaperBackNormal: // 后进纸正常模式，不到纸张结束的逻辑检测纸张时需要检测纸张是否被抽出
				if(((FlagMotorIC&0x30) == 0x30) && (PaperCount<StepBackToPrint*2))
				{
...
//printf("3-----");
			...
			else if(PaperState==sPaperBackNormal)												// 正常状态，能够正常打印
			{
				if(PAPBackToFrontDir)
				{
					...
					if(PaperCount>(StepPageLenth+StepBackToPrint+StepFrontToPrint))
						PaperCount = StepBackToPrint+StepFrontToPrint;
				...
				else // 反方向退纸
				{
					...
					/*else if(SensorRecorderBackIn==0xfe)
					{
						if(PAPMoveStep>StepBackToPrint)
							PAPMoveStep = StepBackToPrint;
						PaperState = sPaperNo; //停止后，等待超时成为无纸状态
					}
					else if(SensorRecorderBackIn==0x00)
						PaperState = sPaperNo;*/
				}
			}
...
//printf("a");
			...
			else if(PaperState==sPaperBackInit)
			{
				...
				else if((SensorRecorderBackOut==0xff)&&(SensorRecorderBackIn==0x7f))	// 1检测到页首，调整步数到达页首
				{
					//PaperCount = 1;
					PAPMoveStep = StepBackToPrint+PAPVolatileStep;
				}
				else if(PAPMoveStep<PAPVolatileStep)									// 0走纸即将结束，仍没能找到页头，进入纸张错误状态，必须人工干预
				{
					PaperState = sPaperErr;
			...
			else if(PaperState==sPaperBackHead)
			{
