...
#if 0
void DealData(volatile unsigned char data)
{;}
#endif
...
**************************************************/
unsigned char FlagIOStatus1 = 0xf8;
// 7:-	6:-		5:LedLine		4:LedErr		3:-	2:Power	10:-



unsigned short FlagHeadIC = 0x0000;
...
unsigned char HeadState = 0xff;
unsigned char FlagPrinter = FlagPRTFast;
...
// 马达驱动相关
unsigned char FlagMotorMode = 0x00;
/*#
...
unsigned char 	CRGState				= 0xff;		// 用于记录初始化等各种状态，全都采用状态机制进行信号量等待 0x55:初始化完成
unsigned char	cCRGSpeed				= 0;		// 用于马达的加减速使用
unsigned short	CRGStopTimePosition		= 0;		// 期望停止时间片位置：
unsigned short	CRGNowTimePosition		= 0;		// 字车当前时间片位置
...
unsigned char PAPState 				= 0xff;
unsigned char cPAPSpeed 			= 0;		// 用于马达的加减速使用
unsigned short PAPMoveStep 			= 0;		// 走纸步数  +向前走纸  -向后退纸  0停止
...
unsigned int cBufferPrintLineWrite = 0;	// 打印缓冲区当前写入行记录
unsigned int cBufferPrintDataWrite = 0;	// 打印缓冲区当前写入行的数据写入位置记录
unsigned int cBufferPrintLineRead = 0;	// 打印缓冲区当前打印行记录
unsigned int cBufferPrintDataRead = 0;	// 打印缓冲区当前打印行的数据读取位置记录
