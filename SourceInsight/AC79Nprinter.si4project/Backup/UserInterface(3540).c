...
// 0x00:尚未配置 	0x01:开始配置 		0x02:未发现网络	0x03:呼叫失败	0x04:呼叫超时	0x05:连接失败	0x06:连接超时
...
volatile unsigned short  StepPageLenth			= StepPageLenthFront;//1584;	// StepBetweenLine*PageLineNumINIT
...
void PAPPhase(void)
{
	...
	if(	((PaperState == sPaperFrontNormal)&&(PAPBackDir))	||
		((PaperState == sPaperBackNormal)&&(PAPFeedDir))	)
	{
		PaperCount++;
	}
	else if(	((PaperState == sPaperFrontNormal)&&(PAPFeedDir))	||
				((PaperState == sPaperBackNormal)&&(PAPBackDir))	)
	{
		if(PaperCount)
			PaperCount--;
	}
