	...
	if(FlagKeyPadNo == 0x0000)
	{
		if((PaperState==sPaperFrontNormal)||(PaperState==sPaperBackNormal))
		{
			if(BufferPrintLineWrited()==0)
			{
				...
				else if(PAPMoveStep == 0)
				{
					...
					{
						PAPMoveStep = StepPageLenth+StepBackToPrint+StepFrontToPrint+StepToCutPos;
						PAPFrontToBackSet
		...
		else if(PaperState==sPaperNo)
		{
			...
			else if(PAPMoveStep==0)
			{
				...
				{
					PAPMoveStep = StepBackToPrint+StepFrontToPrint+StepToCutPos*2;
					PAPBackToFrontSet
...
//printf("a");
			...
			else if(PaperState==sPaperBackNoIn)
			{
