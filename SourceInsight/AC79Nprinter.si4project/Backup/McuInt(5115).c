...
printf("PaperState:  %d  PaperCount: %d \r\n", PaperState, PaperCount);	
	...
	//	printf("PAPState:0x%x-PrintingState:%d-PaperState:%d-FlagMotorIC:0x%x-\r\n", PAPState, PrintingState, PaperState, FlagMotorIC);
...
//printf("FN-");
				if(PaperCount<StepFrontToPrint*2)
				{
					if(adc_get_value(AdcChFrontIn) < ADC4F8)
						c16ForPAP = 0;
					else if(c16ForPAP<50*2)
					{
						PaperState = sPaperNo;
					}
				}
...
if(PAPRunning)
	...
	case sINIT:
		...
		{
			// 配合纸张初始化动作
			if((PAPState!=sPaperFrontNormal)&&(PAPState!=sPaperBackNormal))
			{
				FlagSystemPrintClr
		...
		if(CRGNowTimePosition == CRGStopTimePosition)
		{
			if((cBufferPrintLineRead != cBufferPrintLineWrite) && ((BufferPrint[cBufferPrintLineRead].cLeft < BufferPrint[cBufferPrintLineRead].cRight)) &&((PaperState==sPaperFrontNormal)||(PaperState==sPaperFrontNormal)))
			{
				...
				else
				{
					...
					{
						if((CRGNowTimePosition-BufferPrint[cBufferPrintLineRead].cLeft+CRGNowTimePosition) < (BufferPrint[cBufferPrintLineRead].cRight))
						{
							CRGStopTimePosition = BufferPrint[cBufferPrintLineRead].cLeft;
							CRGLeftSet
						}
