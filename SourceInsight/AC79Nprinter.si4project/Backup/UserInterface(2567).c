...
volatile unsigned char 	CRGState	= 0x55;//			= 0xff;		// 用于记录初始化等各种状态，全都采用状态机制进行信号量等待 0x55:初始化完成
...
void SendCmd(unsigned char CMD1, unsigned char CMD2)
{
 ...
 void InitialIO(void)
 {
	...
	gpio_set_pull_up(IO_PORTC_09,1);
	gpio_set_pull_down(IO_PORTC_09, 0);
	gpio_set_direction(IO_PORTC_09,0);
...
#else
	 gpio_set_die(IO_PORTC_10, 1);
	 gpio_set_pull_up(IO_PORTC_10, 0);
	 gpio_set_pull_down(IO_PORTC_10, 0); // 10k out
	 gpio_set_direction(IO_PORTC_10, 1);
 #endif
 }
...
#if 0
#define AdcChKey	AD_CH_PC10//AD_CH_PA07
#define AdcChFO		AD_CH_PB07
#define AdcChBO		AD_CH_PC00
#define AdcChBI		AD_CH_PC01
#define AdcChFI		AD_CH_PC09
...
//	adc_add_sample_ch(AdcChFO); //AdcFO
//	adc_add_sample_ch(AdcChBO); //AdcBO
//	adc_add_sample_ch(AdcChBI); //AdcBI
//	adc_add_sample_ch(AdcChFI); //AdcFI
	os_time_dly(100);
	u16 adc_val = 0;
    u16 io_vol = 0;
	...
	while(CRGState != 0x55)
	{
		...
		{
			MotorEnable;
			for(c16c1 = 0; c16c1 < BufferPRTSize; c16c1++)
			{
				;//*(BufferPRTA+c16c1) = 0x00;
				//*(BufferPRTB+c16c1) = 0x00;
			}
			...
			{
				CRGNowTimePosition = TimePositionAll-1;
	...
	/*if(USBin)
		...
		if(!KeyPower)
		{
			if((FlagMotorIC&0x30) == 0x30)
			{
				...
				/*if(CRGNowTimePosition)
					...
					CRGStopTimePosition = TimePositionAll-1;
				CRGLeftSet
				CRGCurrent(2);*/

			//	adc_val = adc_get_value(9);//(AD_CH_PA07);
			//	io_vol = adc_get_voltage(9);//(AD_CH_PA07);
			//	printf("adc_val = %d  >>>  %d mv\n", adc_val, io_vol);
			}
		}

		adc_val = adc_get_value(AdcChKey);//(AD_CH_PA07);
		io_vol = adc_get_voltage(AdcChKey);//(AD_CH_PA07);
		//printf("adc_val = %d  >>>  %d mv\n", adc_val, io_vol);
		if(adc_val < 400) // KeyAdc
			;
		else if(adc_val < 750) // KeyAdc
		{
			if((FlagMotorIC&0x30) == 0x30)
			{
				printf("adc_val_L = %d  >>>  %d mv\n", adc_val, io_vol);
				if(LedLine)
					LedLineOpen
				else
					LedLineClose
		...
		else //if(adc_val > 750) // KeyAdc
		{
			if((FlagMotorIC&0x03) == 0x03)
			{
				...
				for(c16c1 = 0; c16c1 < BufferPRTSize; c16c1++)
				{
					*(BufferPRTA+c16c1) = 0x00;//0xff;
					*(BufferPRTB+c16c1) = 0x00;//0xff;
				}
