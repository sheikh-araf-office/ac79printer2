...
unsigned long Tim_PAP_Handler(void)
{
	...
	{
	case sINIT:
		if((cBufferPrintLineRead != cBufferPrintLineWrite) && (BufferPrint[cBufferPrintLineRead].Step) && (!CRGRunning))//&&((PaperState==sPaperFrontNormal)||(PaperState==sPaperFrontNormal)))
		{
			if(CRGState==0x55)
