...
void Tim_Sys_Handler(void)
{
	...
	else if(PowerState == 20)// 1s内松开按键没有影响
	{
		if(KeyPower)
		{
			LedErrClose
	...
	else if(PowerState == 21)// 2s闪烁led等待人工关机确认
	{
		MainPowerOff
	...
	if(FlagKeyPadNo == 0x0000)
	{
		...
		if((KeyLineRecorder == 0xff) && (KeyLineDownTime > 50))
		{
			if(BufferPrintLineWrited() < BufferPrintLineFull)
			{
				BufferPrint[cBufferPrintLineWrite].Step = 32;
	...
	case sPaperNo:
		...
		{
			if((SensorRecorderBackOut)||(SensorRecorderBackIn))
			{
		...
		// 动态检测前进纸装纸动作
		if(SensorRecorderFrontOut == 0x0f)
		{
			if(BufferPrintLineWrited() < BufferPrintLineFull) // 本次不能更换指针，需要配合初始化动态调整走纸步数
			{
	...
	case sPaperFrontInit:
		if(SensorRecorderFrontIn == 0x01)
		{
			if(BufferPrintLineWrited() < BufferPrintLineFull)
			{
				BufferPrint[cBufferPrintLineWrite].Step = StepFrontToPrint;
				BufferPrint[cBufferPrintLineWrite].Dir = 1;
...
/*		ADCKEY = adc_get_value(AdcChKey);
		...
		printf("PowerState:  %s  CRGState: %s \r\n", PowerState, CRGState);
	}
#endif
//return;
