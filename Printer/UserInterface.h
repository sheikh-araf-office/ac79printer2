...
// Brand or debugging identifier, prioritized adjustment is required when adjusting the program
// 0: Debug software version
// 1: SHININGER 2: SANKYO 3: TEJIE 4: COMET 5: INSPUR
// 6: SHIYI 7: SANGONG 8: TEJIE 9: KEMI 10: LANGCHAO
// 255: KAIGE
#define DebugAlkeLOGO      0
#define DebugOnlyData      0    // Only USB/WIFI/SPP/BLE communication, no data parsing and processing, etc.
#define DebugCRGHome       0    // Do not check HOME position during startup
#define DebugPaper         0    // Do not check paper sensor during startup

#define DebugChannelRerutn 0    // Each channel returns data, USB/BLE does not pass data back through its own channel
#define DebugMsg           0    // Use serial port to return debugging information
// Return command
#define SendCmd            0


//#define CONFIG_STATIC_IPADDR_ENABLE // Alke 20230713 Remember the IP assigned by the router, directly use the remembered IP next time to save DHCP time // @20230712 Alke Default closed


#define SM720_K_V11        0    // LedErr and LedLine are reversed on this keypad

#define PrinterDPI 180 //
#define PrinterWide 80//82 // wide column
...
// Power
#define	KeyPower		(gpio_read(IO_PORTB_01)==0)	// 1:key power on/off
...
#define HdGoutHigh		JL_PORTA->OUT &= ~0x08//JL_PORTA->OUT |= 0x02
#define HdGoutLow		JL_PORTA->OUT |= 0x08//JL_PORTA->OUT &= ~0x02
#define HdRckHigh		JL_PORTA->OUT |= 0x02//JL_PORTA->OUT |= 0x04
#define HdRckLow		JL_PORTA->OUT &= ~0x02//JL_PORTA->OUT &= ~0x04
#define HdSckHigh		JL_PORTA->OUT |= 0x04//JL_PORTA->OUT |= 0x08
#define HdSckLow		JL_PORTA->OUT &= ~0x04//JL_PORTA->OUT &= ~0x08
#define HdClrHigh		JL_PORTA->OUT &= ~0x10//JL_PORTA->OUT |= 0x10
#define HdClrLow		JL_PORTA->OUT |= 0x10//JL_PORTA->OUT &= ~0x10
...
#define PAPFrontToBackDir			(FlagMotorMode&0x40)	// 以用户正常使用的角度来定义，靠近身体一侧为前，远离身体一侧为后
...
extern volatile unsigned char HomeRecorder;
...
extern const unsigned char TABLEHardWare[];
extern const unsigned char TABLESoftWare[];
extern const unsigned char TABLEInterface[];
extern const unsigned char TABLECld[];
extern const unsigned char TABLEPid[];
extern const unsigned char TABLEMac[];
extern const unsigned char TABLESsid[];
extern const unsigned char TABLEPwd[];
extern const unsigned char TABLEIp[];
extern const unsigned char TABLESeftPrint[];
//extern const unsigned char TABLESeftPrintSmall[];
//extern const unsigned char TABLESeftPrintBig[];
extern const unsigned char TABLEEnd[];
...
extern volatile unsigned char DataFromChannel;

extern volatile unsigned char ReturnMsg;
...
#define	BufferPrintLineNum				10//2//10//5//5 // 8/10打印数据2880个时，会在越界时出错
#define	BufferPrintLineFull				6//1//6//3
struct sBufferPrint
{
...
extern volatile unsigned char FlagPrintPause;


extern volatile unsigned int PAPContinueStep;
extern volatile unsigned char CRGFlattenPaper;

extern volatile unsigned int StepToCutPos;
#define StepToFlattenPos	550// 570
#define StepToCutPosA		420//420	精简款：最老无托纸板的外壳
#define StepToCutPosB		420//420	经典款：有托纸板大外壳
#define StepToCutPosC		220//220	时尚款：有托纸板小外壳
#define StepFrontToPrint	250//250//141 // 还需要进一步测量数据
#define StepBackToPrint		370 // 还需要进一步测量数据
...
#define StepPageLenthFront 1650 // A4纸: 297mm/0.18mm=1650
#define StepPageLenthBack 1224  //1584 // 链孔纸: 144DPI:72*22=1584	180DPI:90*22=1980
...
#define	HeadDelayCountINIT			25//35//2
...
struct sCommand
{
 	...
 	volatile unsigned char SizeSave[3];	// 存储的命令解析数据及个数，首位位数据个数
	volatile unsigned char State;	// 具体的命令处理时，该条命令状态、步骤、分支等记录
	volatile unsigned int Size;		// 具体的命令处理时，该条命令后面数据个数
};
extern struct sCommand Command;
// SaveData[0]: 存储数据的个数，第一个字节解析后如果不对直接丢弃，因此不需要存储	[1]：第2个解析的字节 [2]：第3个解析的字节
// 二、三级命令解析，都会存储相应的数据，以便在分级处理时，如果数据异常，需要丢掉第2级数据，3级数据进位，本数据进入3级，然后从头开始解析
// 如果命令顺利解析完成，则需要清除存储数据，以便下次从头开始；这是数据正常的操作流程
