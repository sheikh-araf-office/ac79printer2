...
#if 0
// @20230713 Alke USB passback is already possible in this application
...
/**************************************************
For output ports, the port status cannot be directly read to obtain the current status.
  (The level status may not be reliably identified due to a short circuit in the external circuit or excessive load)
  For input ports, the port status cannot be simply read to obtain the current status.
  (It may be that effective changes cannot be recognized due to external circuit interference, especially the buttons)
**************************************************/
...
// 0/1: Power-on process	10: Normal working state	20/21: Power-off process
...
// 0		1/4		1/2		3/4		4/4
// NO		*		KeyLine	*		KeyPaper
// Keys are read in the system interrupt function, continuous status is recorded here for key judgment.
// T = 50ms; 	1111 1111:H	111* *000:Down	0000 0000:L	000* *111:Up
// When using edge events, it is necessary to wait for the end of the event, otherwise the event may be responded to multiple times in one scan cycle.
...
const unsigned char TABLEHardWare[] =
{
	"               HardWare : NeedlePrinter_AC7911B8_WIFI  SM720_V12_Alke_20231027 ", // \n
	//"               HardWare:WIFI Printer SM720-M V12 20231027 \n",
	//"               SoftWare: NeedlePrinter_AC7911B8_WIFI \n",
	//"               USB+WIFI+BL Support! \n\n"
};
const unsigned char TABLESoftWare[] =
{
	"               SoftWare : NeedlePrinter_AC7911B8_WIFI  NP791_V10_Alke_20240107 "
};
const unsigned char TABLEInterface[] =
{
	"               Interface: USB+WIFI+BL "
};
const unsigned char TABLECld[] = // 成列度
{
	"               CLD : "
};
const unsigned char TABLEPid[] =
{
	"               PID : "
};
const unsigned char TABLEMac[] =
{
	"               MAC : "
};
const unsigned char TABLESsid[] =
{
	"               SSID: "
};
const unsigned char TABLEPwd[] =
{
	"               PWD : "
};
const unsigned char TABLEIp[] =
{
	"               I P : "
};
/*const unsigned char TABLESeftPrintSmall[] =
{
	"               abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz"
};
const unsigned char TABLESeftPrintBig[] =
{
	"               ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ"
};*/
const unsigned char TABLEEnd[] =
{
	"               Seft Print End! "
};
...
// 0x00: Not configured yet
// 0x01: Configuration started
// 0x02: Network not found
// 0x03: Call failed
// 0x04: Call timeout
// 0x05: Connected, obtained IP
...
// This printing buffer array serves as the communication data for paper feeding and printing events in the timer interrupt, as well as the command parsing and processing function after receiving data.
//Both communication channels rely entirely on this buffer, and synchronization between the two is achieved through related flags.
//Since dot matrix printing is much slower than data transmission speed, printing begins as soon as 16 lines of data are received,
//without the need to wait for more data as with thermal printing.
...
/*unsigned char MotorIC(void)
{
	...
	else if(MotorICState < 10)
// This function loops for a certain number of times to create a delay. The actual duration of the delay depends on the length of the operating system's time slice.
	{
		MotorICState++;
	}
	else
...
{
//	adc_init(); //@202402211737 Bangladesh 屏蔽此句话就能够找到蓝牙BT SPP
...
void FlashDataRead(void)
{
	...
	{
		;// PID默认必须是0x1508   c232: TM690	// 600d: TD630
		//sDeviceDescriptor[10] = 0x08;//0x0d; //0x32; //
		//sDeviceDescriptor[11] = 0x15;//0x60; //0xc2; //
	}
	else
	{
