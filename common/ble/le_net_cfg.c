    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
                ...
                printf("sangC:HCI_SUBEVENT_LE_CONNECTION_COMPLETE: %0x\n", con_handle);
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            printf("sangC:HCI_EVENT_DISCONNECTION_COMPLETE: %0x\n", packet[5]);
            con_handle = 0;
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    log_info("sangC:read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);
...
*/
hci_con_handle_t HCI_le_Printer;
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
  int result = 0;
  u16 tmp16;
  ...
  switch (handle)
  {
	...
	case ATT_CHARACTERISTIC_49535343_ACA3_481C_91EC_D85E28A60318_01_CLIENT_CONFIGURATION_HANDLE:
	case ATT_CHARACTERISTIC_FEC8_01_CLIENT_CONFIGURATION_HANDLE:
	case ATT_CHARACTERISTIC_FF81_01_CLIENT_CONFIGURATION_HANDLE:
  	    set_ble_work_state(BLE_ST_NOTIFY_IDICATE);
        check_connetion_updata_deal();
        log_info("\n------write ccc:%04x,%02x\n", handle, buffer[0]);
        att_set_ccc_config(handle, buffer[0]);
    ...
    case ATT_CHARACTERISTIC_49535343_6DAA_4D02_ABF6_19569ACA69FE_01_VALUE_HANDLE://WRITE    | DYNAMIC
    case ATT_CHARACTERISTIC_49535343_8841_43F4_A8D4_ECBE34729BB3_01_VALUE_HANDLE://WRITE    | WRITE_WITHOUT_RESPONSE |DYNAMIC,
	case ATT_CHARACTERISTIC_49535343_ACA3_481C_91EC_D85E28A60318_01_VALUE_HANDLE://WRITE    | NOTIFY   | DYNAMIC
	case ATT_CHARACTERISTIC_FEC7_01_VALUE_HANDLE:
	case ATT_CHARACTERISTIC_FF82_01_VALUE_HANDLE:
	   // printf("SangBle:%d\n",buffer_size);
		LESaveData(buffer_size,buffer);	//
		HCI_le_Printer=connection_handle;
		if(LE_RevC->CheckBufferEmptySize()&&LE_RevC->Waitting==false)
		{

			LE_RevC->Waitting=true;
			att_server_flow_hold(HCI_le_Printer,1);//LE disable data in
        #if DebugMsg
			printf("sang:BLE busy set:%d..............................\r\n",LE_RevC->BuferGetEmptySize());
        #endif
		}
	    /*
	  USBSaveData(buffer_size,buffer);//保存数据
	  //printf("size:%d\r\n",buffer_size);
	  if(GetEmptySize()<BusyLevelBLE&&BTI.BLEState==NormalState)
	  {
		  BTI.BLEState=BusyState;//设置状态
		  printf("BLE Busy Set:%d-%d,%d\r\n",GetEmptySize(),con_handle,connection_handle);
		  //att_server_flow_enable(1);
		 // att_server_flow_hold(con_handle,1);
	  }
        */
      break;
    default:
...
#define ATT_DATA_RECIEVT_FLOW           1//流控功能使能
/*-----------------------------------------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------------------------------------*/
void bt_ble_init(void)
{
    log_info("sangPPP:***** ble_init******\n");
...
#if DebugMsg
    log_info("sang:att_server_flow_enable-----------\n");
#endif
    att_server_flow_enable(1);
