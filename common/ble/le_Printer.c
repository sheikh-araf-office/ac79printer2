...
//	transport_spp_flow_cfg(); // @202402181431 Alke 按照桑哥建议增加
}
void LESaveData(unsigned long  sizebytes,unsigned char *BufferReceive)
{
    unsigned int i;
	...
	if((LE_RevC->Pwrite+sizebytes)<=LEMAXBufferReceiveSize)
	{
		...
		if((LE_RevC->Pwrite+sizebytes)==LEMAXBufferReceiveSize)
			...
			LE_RevC->Pwrite=LE_RevC->Pwrite+sizebytes;
	}
	else
	{
		memcpy(&LE_RevC->PBufferRev[LE_RevC->Pwrite],BufferReceive,LEMAXBufferReceiveSize-LE_RevC->Pwrite);
		memcpy(LE_RevC->PBufferRev,&BufferReceive[LEMAXBufferReceiveSize-LE_RevC->Pwrite], LE_RevC->Pwrite+sizebytes-LEMAXBufferReceiveSize);
...
unsigned long LEBufferGetEmptySize(void)
{
	...
	{
		return LEMAXBufferReceiveSize+LE_RevC->Pread-LE_RevC->Pwrite;
	}
	else
	{
...
unsigned char LECheckBufferEmptySize(void)
{
	...
	//if(LEBufferGetEmptySize()<LERevBusyLevelValue) // @Alke @20231018
	{
		return true;
	}
	return false;
...
{
	// Alke @20230726 There cannot be BLE return data here. There are too many BLE return channels and the cooperation of the APP is required. It is quite complicated and will not be processed yet.
	DealData(data);

	// @20240201 After several tests, the BLE transmission speed is too slow and is no longer used.
...
void LEDataProcesser(void)
{
	...
	while(1)
	{
		...
		// R==W: 空
		if(LE_RevC->Pread==LE_RevC->Pwrite) //空的返回
			return;
		else								// 有数据
		{
			// 只有有数据进来，才会固定当前数据通道
			if((DataFromChannel!=0)&&(DataFromChannel!=4))
				return;
			DataFromChannel = 4;
