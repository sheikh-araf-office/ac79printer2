...
#endif
extern Device_Rev *USB_RevC; // Alke @20230726 再次按照之前的添�?
static void hid_endpoint_init(struct usb_device_t *usb_device, u32 itf)
{
    const usb_dev usb_id = usb_device2id(usb_device);

    // Alke @20230726 实际测试，这两句包括前函数边的声明对于USB通讯没有影响
	USB_RevC->IDHandle=usb_device2id(usb_device); // Alke @20230726 再次按照之前的添�?
	USB_RevC->EP=HID_EP_IN; // Alke @20230726 再次按照之前的添�?
